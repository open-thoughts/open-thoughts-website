...
In summary, we observe that open reasoning models have a severe lack of robustness to simple reasoning problems found in Alice in Wonderland. This lack of ability is not currently captured by the standard benchmarks (AIME, MATH, GPQA) the community is using to evaluate reasoning models. There is much more work to be done to close the gap between the best open reasoning models and the best closed reasoning models.
