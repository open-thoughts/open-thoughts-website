...
Dataset size is a powerful metric for improving downstream quality. Using the above insights and increasing the dataset scale to 100K, we generate the SOTA open-data 32B agentic model. Across dataset scales, we see our dataset is the only one competitive on both general agentic and coding capabilities.
